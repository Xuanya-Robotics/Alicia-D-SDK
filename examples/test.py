...
def main():
    ...
    controller = ArmController(port="/dev/cu.usbserial-1440", baudrate=115200, debug_mode=True)
    ...
    try:
        ...
        while True:
            ...
            # 打印串口收到的所有原始字节（不做任何协议解析）
            # controller.serial_comm.draw_frame()

            # 短暂延时
            data = controller.serial_comm.serial_port.read(64)
            if data:
                print("Raw:", data.hex())
            time.sleep(0.1)
