...
from robolab.coord import convert_ori_format, convert_quat_order
...
def get_ik_from_chain(chain, goal_pose: Union[torch.Tensor, None, List, Tuple], device, goal_in_rob_tf: bool = True,
    ...
    lim = torch.tensor(chain.get_joint_limits(), device=device, dtype=torch.float32)

    if cur_configs is not None:
        cur_configs = torch.tensor(cur_configs, device=device, dtype=torch.float32)
