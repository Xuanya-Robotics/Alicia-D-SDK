...
import robolab.coord.rotation_conversions as tRot
import rofunc as rf
# Configure logging
...
class Humanoid_Batch:
    ...
    def __init__(self, cfg, device=torch.device("cpu")):
        self.cfg = cfg
        self.root_dir = rf.oslab.get_rofunc_path("simulator/assets")
        self.mjcf_file = os.path.join(self.root_dir, cfg.asset.assetFileName)
        ...
        if hasattr(cfg, "extend_config"):
            for extend_config in cfg.extend_config:
                ...
                self._parents = torch.cat(
                    [self._parents.to(device), torch.tensor([self.body_names.index(extend_config.parent_name)]).to(device)], dim=0)
    ...
    def mesh_fk(self, pose=None, trans=None):
        ...
        for geom in geoms:
            ...
            body_trans = g_trans[body_idx].detach().cpu().numpy().copy()
            body_rot = g_rot[body_idx].detach().cpu().numpy().copy()
