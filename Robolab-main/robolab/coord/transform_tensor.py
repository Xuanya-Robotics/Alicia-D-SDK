...
def check_rot_matrix_tensor(rot_matrix):
    ...
    >>> from robolab.coord.transform import random_rot_matrix
...
def random_rot_matrix_tensor(batch_size, rand=None):
    ...
    True
    >>> from robolab.coord.transform import check_rot_matrix
...
def quat_mul(quat1, quat2):
    """
    Perform quaternion multiplication between two quaternions.

    :param quat1: First quaternion (batch, 4) or (4,)
    :param quat2: Second quaternion (batch, 4) or (4,)
    :return: The result of quaternion multiplication as a new quaternion (batch, 4) or (4,)
    """
    # Ensure both quaternions are normalized
    quat1 = check_quat_tensor(quat1)
    quat2 = check_quat_tensor(quat2)

    # Extract the components of both quaternions
    x1, y1, z1, w1 = quat1[..., 0], quat1[..., 1], quat1[..., 2], quat1[..., 3]
    x2, y2, z2, w2 = quat2[..., 0], quat2[..., 1], quat2[..., 2], quat2[..., 3]

    # Compute the quaternion multiplication using the formula
    x = w1 * x2 + x1 * w2 + y1 * z2 - z1 * y2
    y = w1 * y2 - x1 * z2 + y1 * w2 + z1 * x2
    z = w1 * z2 + x1 * y2 - y1 * x2 + z1 * w2
    w = w1 * w2 - x1 * x2 - y1 * y2 - z1 * z2

    # Stack the result back into a single tensor
    return torch.stack((x, y, z, w), dim=-1)


def pos_rotate(pos, quat):
    """
    Rotate the position by the quaternion.

    :param pos: (batch, 3) or (3,)
    :param quat: (batch, 4) or (4,)
    :return: The rotated position as a new position (batch, 3) or (3,)

    >>> pos_rotate([1, 0, 0], [0, 0, 0, 1])
    tensor([[1., 0., 0.]])
    >>> pos_rotate([[1, 0, 0]], [[0, 0, 0, 1]])
    tensor([[1., 0., 0.]])
    >>> pos_rotate(np.array([1, 0, 0]), np.array([0, 0, 0, 1]))
    tensor([[1., 0., 0.]])
    """
    pos = check_pos_tensor(pos)
    quat = check_quat_tensor(quat)
    ...
    return torch.matmul(pos, rot_matrix.transpose(-1, -2))