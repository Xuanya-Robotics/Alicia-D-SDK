...
class BBOPlannerBi:
    def __init__(self, args, rdf_model, box_size, box_pos, box_rotation, used_links=None):
        """
    ...
    def bi_optimizer(self, p_l, p_r, n_l, n_r, theta_mid, base_trans=None, batch=64):
        joint_value = self.theta_min + torch.rand(batch, self.rdf_bp.robot.num_joint).to(self.device) * (
                self.theta_max - self.theta_min)
        valid_theta_list = []
        num_accept = 0
        while True:
            c_reaching_l, J_reaching_l, dist_l = self.reaching_cost(joint_value, p_l, base_trans=base_trans)
            c_reaching_r, J_reaching_r, dist_r = self.reaching_cost(joint_value, p_r, base_trans=base_trans)
            c_collision, J_collision, penetration = self.collision_cost(joint_value, self.object_internal_points,
                                                                        base_trans=base_trans)
            c_normal_l, J_normal_l = self.normal_cost(joint_value, p_l, n_l, base_trans=base_trans)
            c_normal_r, J_normal_r = self.normal_cost(joint_value, p_r, n_r, base_trans=base_trans)
            ...
            c_l = torch.cat([c_reaching_l * 10.0, c_collision * 1.0, c_joints_limits * 10.0, c_joints_middle * 0.1],
                            dim=1)
            J_l = torch.cat([J_reaching_l * 1.0, J_collision * 1.0, J_joints_limits * 1.0, J_joints_middle], dim=1)
            c_r = torch.cat([c_reaching_r * 10.0, c_collision * 1.0, c_joints_limits * 10.0, c_joints_middle * 0.1],
                            dim=1)
            J_r = torch.cat([J_reaching_r * 1.0, J_collision * 1.0, J_joints_limits * 1.0, J_joints_middle], dim=1)
            ...
            # accept = (dist_l < 0.005) * (dist_r < 0.005) * (penetration < 0.01) * (c_normal_l < 0.2) * (
            #             c_normal_r < 0.2) * joint_accept
            accept = (dist_l < 0.005) * (dist_r < 0.005) * (penetration < 0.01) * joint_accept
            ...
            if accept.sum() > 0:
                ...
                d_theta_l = (torch.matmul(torch.linalg.pinv(-J_l), c_l.unsqueeze(-1)) * 10. * 0.01)[:, :,
                            0]  # Gauss-Newton
                d_theta_r = (torch.matmul(torch.linalg.pinv(-J_r), c_r.unsqueeze(-1)) * 10. * 0.01)[:, :,
                            0]  # Gauss-Newton
                # d_theta = torch.clamp(d_theta_l, -0.05, 0.05)
                joint_value += d_theta_l[~accept]  # Update state
                joint_value += d_theta_r[~accept]  # Update state
                joint_value = self.limit_angles(joint_value)
                if len(valid_theta_list) >= self.num_results:
                ...
                d_theta_r = (torch.matmul(torch.linalg.pinv(-J_r), c_r.unsqueeze(-1)) * 10. * 0.01)[:, :,
                            0]  # Gauss-Newton
                # d_theta = torch.clamp(d_theta_l, -0.05, 0.05)
                joint_value += d_theta_l
                joint_value += d_theta_r  # Update state
                joint_value = self.limit_angles(joint_value)
